    ...
    def lam(luot: LuotChay, tt: TrangThaiKhau):
        ...
        if not tu_lieu and link:
            ...
            if lay is None:
                ...
                lay = lay_script
            # `goc`/`kenh` để `lay_script` đọc KHO LỜI THOẠI trước khi gọi
            # mạng (`core/loi_thoai.py`): lúc 02:00 không có trình duyệt nào
            # mở, và YouTube đang chặn địa chỉ VPS này theo số lượt hỏi — chữ
            # phiên trình duyệt sáng qua đã hút về là đường duy nhất không tốn
            # lượt hỏi nào. Kho rỗng thì `lay_script` đi y nguyên đường cũ.
            ket = lay(link, cancel=bc.cancel, cho_phep_nghe=True,
                      ngon_ngu_uu_tien=k.ngon_ngu, on_log=bc.on_log,
                      goc=bc.goc, kenh=(luot.ma_kenh or getattr(k, "ma", "")))
