...
__all__ = ["boi_canh_ssl", "dang_json", "mo_url", "tai_bytes", "UA"]
...
def dang_json(url: str, khoi, *, cho: float = 10.0,
              headers: Optional[Dict[str, str]] = None) -> None:
    """GỬI một khối JSON đi bằng POST, cũng qua đúng bộ chứng chỉ ở trên.

    `mo_url`/`tai_bytes` là đường LẤY VỀ. Báo động thì ngược lại: nó ĐẨY một
    dòng chữ ra Telegram hoặc webhook của khách (xem `core/bao_dong.py`).
    Không có cửa đẩy đi thì nơi cần gửi sẽ tự gọi `urlopen` trần — đúng thứ
    `tests/test_mang_an_toan.py` cấm, và cấm có lý: đường đẩy cũng mang theo
    khoá bot trong thân yêu cầu, chen giữa được thì đọc được khoá ấy.

    Chỉ quan tâm gửi được hay không, nên không trả về thân phản hồi: nơi gọi
    chỉ cần biết có ném lỗi hay không.
    """
    du_lieu = json.dumps(khoi, ensure_ascii=False).encode("utf-8")
    dau = {"Content-Type": "application/json; charset=utf-8"}
    dau.update(headers or {})
    with mo_url_post(url, du_lieu, cho=cho, headers=dau):
        pass


def mo_url_post(url: str, du_lieu: bytes, *, cho: float = 10.0,
                headers: Optional[Dict[str, str]] = None):
    """Như `mo_url` nhưng có thân yêu cầu — tức là POST."""
    if not str(url).startswith("https://"):
        raise ValueError("Chỉ gửi qua HTTPS: {0}".format(url))
    dau = {"User-Agent": UA}
    dau.update(headers or {})
    yeu_cau = urllib.request.Request(url, data=du_lieu, headers=dau, method="POST")
    return urllib.request.urlopen(  # noqa: S310 — đã chốt https ngay trên
        yeu_cau, timeout=cho, context=boi_canh_ssl())
