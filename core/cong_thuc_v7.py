...
__all__ = ["CAU_HINH_MAC_DINH", "TEP_CAU_HINH", "TEP_SO_CHON", "VideoMinh", "DongV7", "KetQua",
           ...
           "cum_cua_tieu_de", "thanh_tich_cum"]
...
def cum_cua_tieu_de(tieu_de: str, ch: Optional[Dict] = None) -> List[str]:
    """Mã các CỤM CHỦ ĐỀ mà một tiêu đề thuộc về, theo cấu hình `ch` — bản CÔNG KHAI của
    `_cum_cua` (cùng một thân, không nhân bản luật).

    Mở ra vì `core/nhom_kenh.doc_bang_nhom` phải phân loại tiêu đề video ĐÃ ĐĂNG của các
    kênh ANH EM trong nhóm bằng ĐÚNG bộ cụm mà kênh đang chọn nguồn dùng để chấm ứng viên
    (`cham` → `_cum_cua` với chính `ch` này). Nếu bên đọc bảng nhóm tự viết một bộ nhận cụm
    thứ hai thì hai bên nói hai thứ tiếng: "cụm đang thắng ở kênh anh em" sẽ không bao giờ
    trùng tên với cụm của ứng viên, và điểm anh em lặng lẽ luôn bằng 0 — một lỗi không
    cảnh báo, không ai thấy.

    `ch=None` → bộ cụm MẶC ĐỊNH của cả ngách (`CAU_HINH_MAC_DINH`). Đủ để xem/nhìn, nhưng
    nơi CHẤM phải truyền `ch` thật của kênh: cấu hình kênh em (`cau_hinh_cho_tep`) còn có
    thêm các cụm `tep-*` rút từ tuyến con của tệp nó đánh, bản mặc định không có.
    """
    return _cum_cua(tieu_de, ch or CAU_HINH_MAC_DINH)
...
def video_cua_kenh(goc: str, kenh: str, ch: Optional[Dict] = None,
    ...
    ═══ CHỈ VIDEO CỦA ĐỜI NÀY CỦA KÊNH ═══

    Kênh khai `ngay_bat_dau` trong `kenh.yaml` thì video đăng trước mốc bị bỏ HẲN
    khỏi danh sách, trước cả `_danh_dau_thang`. Phải bỏ trước chứ không chỉ bỏ cờ
    thắng: `_danh_dau_thang` lấy TRUNG VỊ hiển thị 48h của cả danh sách làm ngưỡng
    (`boi_so_trung_vi × median`), nên một video đồ chơi 2018 với 35.701 hiển thị
    (`7k6vzNw2uns` của TL4-T7, đo 22/09/2026) vừa tự thắng vừa kéo ngưỡng của mọi
    video tâm lý lên. Bỏ khỏi danh sách cũng đồng thời dọn `thanh_tich_cum` và
    `auto_khau.tieu_de_thang_cua_kenh` — cả hai đọc qua hàm này.
    """
    ch = ch or CAU_HINH_MAC_DINH
    bay_gio = bay_gio or _dt.datetime.utcnow()
    thu_muc = thu_muc_chi_so(goc, kenh)
    moc = _loc.moc_cua_thu_muc(duong_kenh(goc, kenh))
    but = _loc.but_nhat_ky(thu_muc)
    ra: List[VideoMinh] = []
    try:
    ...
    for ma in ten_con:
        ...
        if len(ma) != 11 or not os.path.isdir(duong):
            continue
        if moc and not _loc.trong_pham_vi(
                moc, _loc.ngay_dang_cua_video(duong), ghi=but, nhan=ma):
            continue
        vm = VideoMinh(ma=ma)
...
def da_co_video_thang(goc: str, kenh: str, *, ch: Optional[Dict] = None) -> bool:
    ...
    thu_muc = thu_muc_chi_so(goc, kenh)
    # Cổng chuyển V7 cũng phải bỏ video đời trước của kênh: `7k6vzNw2uns` (đồ chơi
    # 2018, 35.701 hiển thị @ CTR 7,97%) qua mọi ngưỡng `chuyen_v7` một mình, tức
    # nó tự bật cổng cho kênh TL4-T7 sớm hơn sự thật. Xem `loc_video.py`.
    moc = _loc.moc_cua_thu_muc(duong_kenh(goc, kenh))
    but = _loc.but_nhat_ky(thu_muc)
    try:
        ten_con = sorted(os.listdir(thu_muc))
    ...
    for ma in ten_con:
        ...
        if len(ma) != 11 or not os.path.isdir(duong_video):
            continue
        if moc and not _loc.trong_pham_vi(
                moc, _loc.ngay_dang_cua_video(duong_video), ghi=but, nhan=ma):
            continue
