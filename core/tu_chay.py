...
__all__ = ["THU_MUC_TU_CHAY", "duong_bao_cao_ngay", "chay_mot_ngay",
           "co_cau_hinh_v7", "ung_vien_xep_hang",
           "cong_diem_anh_em", "TY_LE_DIEM_ANH_EM", "NGUONG_BAC_VIEW",
...
#:
#: Hai lý do đó cộng lại: tín hiệu anh em là GỢI Ý, không phải mệnh lệnh. Trần một nửa
#: cột cụm (15/100) giữ nó luôn nhỏ hơn thành tích CỦA CHÍNH KÊNH MÌNH — thứ đo bằng số
#: Studio thật của chính kênh, ở đúng mốc, trên đúng tệp của nó.
TY_LE_DIEM_ANH_EM = 0.5
...
def cong_diem_anh_em(goc: str, ma_kenh: str, ds: List[Dict[str, Any]], co_v7_truoc: bool,
    ...
    **(a) Trọng số anh em NHỎ HƠN của chính mình.** Trần = `TY_LE_DIEM_ANH_EM` × cột cụm
    (`trong_so["cum"]`) — một nửa, 15/100. Xem lý do đầy đủ ở `TY_LE_DIEM_ANH_EM`: bốn
    kênh đánh bốn TỆP khác nhau, và "thắng" đọc từ bảng nhóm là xấp xỉ rộng.
    ...
    * **Trước V7** (kênh em mới, xếp theo sức nổ của kênh NGUỒN): không có `diem` so được
    ...
    tran = float((ch.get("trong_so") or {}).get("cum", 30) or 30) * TY_LE_DIEM_ANH_EM
    ...
    for d in ds:
        ...
        d["ly_do"] = list(d.get("ly_do") or []) + [
            "cụm “{0}” đang thắng ở kênh anh em {1} ({2:.0f}%) — cộng {3:g} (trần {4:g}, một nửa "
            "cột cụm)".format(d["anh_em"]["ten_cum"], tot[2], tot[0] * 100, d["diem_anh_em"], tran)]
        so_cong += 1
    ...
    if co_v7_truoc:
        ...
        ds.sort(key=lambda d: (0 if float(d.get("view") or 0) >= NGUONG_BAC_VIEW else 1,
...
def _cong_diem_anh_em_an_toan(goc: str, ma_kenh: str, ds: List[Dict[str, Any]],
                              co_v7_truoc: bool,
                              ghi_log: Callable[[str], None]) -> List[Dict[str, Any]]:
    ...
    """
    try:
        return cong_diem_anh_em(goc, ma_kenh, ds, co_v7_truoc, ghi_log)
...
def ung_vien_xep_hang(goc: str, ma_kenh: str, co_v7_truoc: bool, loai_tru: set,
    ...
    if co_v7_truoc:
        try:
            kq = cham_v7_fn(goc, ma_kenh)
        except Exception as loi:  # noqa: BLE001
            ghi_log("  Công thức V7 hỏng ({0}) — không có nguồn hôm nay (kênh đã có cấu hình V7, "
        ...
        for d in getattr(kq, "ung_vien", None) or []:
            ...
            ra.append({"nguon": "v7", "ma": ma, "link": str(getattr(d, "link", "") or ""),
                       ...
                       # Điểm cụm CỦA CHÍNH KÊNH MÌNH — mang theo để `cong_diem_anh_em`
                       # biết ứng viên nào TỰ THẮNG (trần cho tín hiệu anh em).
                       "diem_cum": float(getattr(d, "diem_cum", 0) or 0),
                       "ly_do": list(getattr(d, "ly_do", None) or [])})
        if not ra:
            ghi_log("  Công thức V7: không có ứng viên nào đạt “{0}” trở lên — không có nguồn hôm nay "
                   "(kênh đã có cấu hình V7, không rơi về bảng Một nút).".format(v7.NEN_LAM))
        return _cong_diem_anh_em_an_toan(goc, ma_kenh, ra, True, ghi_log)

    du_lieu = doc_danh_sach_fn(goc, ma_kenh) or {}
    ...
    def _khoa_uu_tien(item: Tuple[str, str, Dict[str, Any]]) -> Tuple[int, float, float, float]:
        ...
        return (0 if view >= NGUONG_BAC_VIEW else 1, -min(vuot, 25.0), -tang, -view)

    ung_vien.sort(key=_khoa_uu_tien)
    ra = []
    for ma, link, d in ung_vien:
        ...
        ra.append({"nguon": "mot_nut", "ma": ma, "link": link,
                   ...
                   # nó không được đổi thứ tự giữa các bậc (xem `NGUONG_BAC_VIEW`).
                   "view": view, "ly_do": ly_do})
    return _cong_diem_anh_em_an_toan(goc, ma_kenh, ra, False, ghi_log)
...
def _dung_md_tat_ca(ngay_str: str, runs: List[Dict[str, Any]]) -> str:
    ...
    for i, r in enumerate(runs, 1):
        ...
        if don.get("tong_bytes"):
            dong.append("Đã dọn đĩa (thường, video đã đăng quá hạn ân xá): giải phóng {0}"
                       .format(_byte_nguoi_doc(don.get("tong_bytes") or 0)))
