...
def doc_kenh(kenh: str, goc: Optional[str] = None, *,
             moc: Optional[str] = None) -> List[BanGhi]:
    """Đọc toàn bộ lần chụp của một kênh, giải mã cái nào chưa giải mã.

    ═══ CHỈ TRẢ VỀ VIDEO TRONG PHẠM VI CỦA KÊNH ═══

    Kênh khai `ngay_bat_dau` trong `kenh.yaml` thì lần chụp của video đăng TRƯỚC
    mốc ấy bị bỏ — đó là video của đời trước kênh (TL4-T7 còn hai video đồ chơi
    2018, xem `loc_video.py`). Đây là CỬA DUY NHẤT của `bang-tom-tat.csv`, khối
    báo cáo cho AI và `su_that_kenh()`, nên cắm ở đây là ba nơi cùng sạch.

    Gói raw KHÔNG bị xoá: vẫn nằm nguyên trong `chi-so/<id>/`, chỉ không lên bảng.

    `moc` truyền thẳng để test và để nơi gọi đã đọc `kenh.yaml` khỏi đọc lại; để
    `None` thì tự đọc từ `kenh.yaml` của kênh. Rỗng = không lọc gì (nết cũ).
    """
    goc = goc or thu_muc_du_lieu()
    ...
    if not os.path.isdir(kenh_dir):
        return []
    if moc is None:
        moc = _loc.moc_cua_kenh(goc, kenh)
    ...
    but = _loc.but_nhat_ky(kenh_dir)
    ra: List[BanGhi] = []
    for b in tho:
        if b["video_id"] == "kenh":
            continue
        # Bản ghi gộp có thể thiếu `ngay_dang` (mốc chưa giải mã xong) — thử đọc
        # thẳng dữ liệu thô của video trước khi kết luận "không biết ngày".
        ngay = b.get("ngay_dang") or _loc.ngay_dang_cua_video(
            os.path.join(kenh_dir, str(b["video_id"])))
        if not _loc.trong_pham_vi(moc, ngay, ghi=but, nhan=str(b["video_id"])):
            continue
        pool = b.get("pool") or {}
        ra.append(BanGhi(
