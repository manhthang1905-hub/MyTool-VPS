...
def _so_tep(ma: str) -> str:
    """Mã tệp DẠNG SỐ ("1".."4", "8") từ bất kỳ cách viết nào.

    `kenh.yaml` của bốn kênh thật trên VPS khai tệp bằng SLUG
    (`tep: "nguoi-to-mo-xem-minh-la-kieu-nguoi-nao"`), không phải bằng số —
    `core.tuyen_noi_dung` là chỗ giữ bảng số ↔ slug, hỏi thẳng nó. Không có
    bảng ấy (bản cũ, module hỏng) thì trả lại chính chuỗi vào, và
    :func:`mo_ta_tep` lùi về cách cũ.
    """
    ma = str(ma or "").strip()
    if not ma:
        return ""
    try:
        ...
        ma_dai = tn.ma_tep(ma)
        for so, slug in tn._theo_so().items():  # noqa: SLF001 — bảng của chính nó
            if slug == ma_dai:
                return so
    except Exception:  # noqa: BLE001 — thiếu bảng thì dùng nguyên chuỗi vào
        pass
    return ma
...
def mo_ta_tep(ma: str) -> Tuple[str, str]:
    ...
    Nhận cả SỐ ("3") lẫn SLUG ("nguoi-to-mo-xem-minh-la-kieu-nguoi-nao") — bốn
    kênh thật trên VPS khai bằng slug, và cột "Tệp" từng hiện nguyên cái slug
    ấy ra màn hình (`MyTool/CLAUDE.md`: không dùng từ kỹ thuật trên giao diện).
    """
    goc_ma = str(ma or "").strip()
    ma = _so_tep(goc_ma)
...
def _mot_kenh(goc: str, ma: str, cai: Dict[str, Any], *, bay_gio: _dt.datetime,
    ...
    # Bỏ video đời trước của kênh (xem `core/chi_so_ytb/loc_video.py`). Ô "7 ngày"
    # vốn đã tự loại chúng bằng cửa sổ ngày, nhưng lọc ở đây là chốt cho MỌI ô đọc
    # `bang_tt` về sau — thêm một ô "trọn đời" mà quên lọc là màn hình lại nói con
    # số của video đồ chơi 2018.
    try:
        from .chi_so_ytb import loc_video as _loc  # noqa: PLC0415

        bang_tt = _loc.loc_hang_bang(_loc.moc_cua_thu_muc(duong_kenh(goc, ma)), bang_tt)
    except Exception:  # noqa: BLE001 — màn hình không bao giờ được trắng vì bộ lọc
        pass
