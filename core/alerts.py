...
def canh_bao_ra_ngoai(alert: BalanceAlert, *, goc: str, loai: str = "vi_can") -> bool:
    """Cầu nối MỎNG sang :mod:`core.bao_dong` — bắn số dư thấp RA NGOÀI máy.

    Vì sao KHÔNG nhét thẳng vào :func:`assess_balance`/:class:`BalanceWatcher`:
    hai cái đó cố ý THUẦN TUÝ (không gọi mạng) để test bằng số dựng tay, đúng
    như lời hứa ở đầu file. Việc gọi mạng bị đẩy hẳn ra hàm riêng này — nhập
    `core.bao_dong` TRỄ (trong thân hàm) để `core.alerts` không kéo theo phụ
    thuộc mạng/`threading` chỉ vì import module.

    Gọi hàm này ở bất cứ đâu vừa có kết quả :func:`assess_balance` là an toàn,
    kể cả khi mức đang là `OK` (hàm tự bỏ qua) hoặc `bao-dong.json` chưa được
    điền (im lặng, xem :mod:`core.bao_dong`) — không cần bọc if/else riêng.

    Nhánh khác muốn gọi báo động cho sự cố KHÁC (trạm chết, đĩa đầy, lịch bỏ
    lỡ...) thì gọi thẳng ``core.bao_dong.bao_dong(loai, tieu_de, chi_tiet,
    goc=...)`` — hàm này chỉ là lối tắt riêng cho đúng trường hợp số dư.
    """
    if not alert.is_warning:
        return False
    from .bao_dong import bao_dong as _bao_dong  # noqa: PLC0415 — nhập trễ, xem docstring

    return _bao_dong(loai, alert.title, alert.message, goc=goc)
