...
def _extract_mot_nac(url: str, opts: Dict, cancel: Optional[threading.Event],
                     nac) -> Dict:
    """Gọi yt-dlp **một nấc**, thử lại vài lần khi lỗi mạng.
    ...
    Nhưng thử lại chỉ chữa được lỗi *nhất thời*. Gặp dấu chặn-theo-IP thì hỏi
    lại ba lần cũng ra đúng ba lần y hệt — thôi ngay để nhường lượt cho nấc sau
    (cookie/proxy), thứ thật sự có cửa đổi kết quả.
    """
    from .mang_youtube import la_dau_chan_ip, tuy_chon_ytdlp  # noqa: PLC0415
    ...
    base.update(opts)
    # Nấc trần trả về `{}` nên dòng này là không-làm-gì — bản không có
    # `mang-youtube.json` chạy y hệt trước ngày 22/09/2026.
    base.update(tuy_chon_ytdlp(nac))
...
def _extract(url: str, opts: Dict, cancel: Optional[threading.Event] = None, *,
             nac_ra: Optional[List] = None,
             on_log: Optional[Callable[[str], None]] = None) -> Dict:
    """Gọi yt-dlp, **leo dần các nấc** trong `mang-youtube.json`.

    Đây vẫn là **cửa duy nhất** ra YouTube bằng yt-dlp của cả tool, và chữ ký
    ba tham số đầu giữ y nguyên — mọi nơi gọi cũ (`fetch_channel`,
    `search_videos`, `resolve_video_channel`, `core/script_video.py`) không
    phải đổi một dòng, và proxy áp cho tất cả cùng lúc.

    `nac_ra` là một danh sách rỗng nơi gọi đưa vào để **nhận lại cái nấc đã qua
    được**. Cần nó vì nơi gọi còn hai đường mạng nữa phải đi bằng đúng nấc ấy:
    tải tệp phụ đề (`urllib`) và `youtube-transcript-api`. Chúng đi nấc khác
    thì proxy chỉ áp được một nửa — xin địa chỉ tệp phụ đề bằng IP dân cư rồi
    tải tệp ấy bằng chính IP máy chủ vừa bị chặn.

    (Trả qua danh sách chứ không đổi kiểu trả về, vì đổi kiểu trả về là bắt cả
    chục nơi gọi và cả chục bài kiểm đổi theo cho một thông tin mà hầu hết
    chúng không dùng.)

    Không có `mang-youtube.json` → đúng một nấc → hàm này chỉ là một vòng lặp
    chạy một lần, hành vi không đổi một ly.

    Ghi nhật ký từng nấc để lần sau đọc log là biết ngay **nấc nào cứu được**.
    Đó là thứ đáng giá nhất của cả cơ chế này: lần sau chủ dự án không phải đo
    lại từ đầu, chỉ cần đọc một dòng.
    """
    from .mang_youtube import (  # noqa: PLC0415
        cac_nac, giai_thich_loi_cookie, la_loi_cookie)

    def ghi(dong: str) -> None:
        if on_log is not None:
            on_log(dong)

    def bao_nac(nac) -> None:
        if nac_ra is not None:
            nac_ra.append(nac)

    danh_sach = cac_nac()
    for thu_tu, nac in enumerate(danh_sach):
        cuoi = thu_tu == len(danh_sach) - 1
        if not nac.tran:
            ghi("    đường mạng: thử nấc «{0}»…".format(nac.ten))
        try:
            payload = _extract_mot_nac(url, opts, cancel, nac)
        except Cancelled:
            raise
        except Exception as loi:  # noqa: BLE001 — nấc này hỏng thì còn nấc sau
            if cuoi:
                bao_nac(nac)
                raise
            ghi("    nấc «{0}» không qua: {1}".format(nac.ten, str(loi)[:120]))
            if nac.ho_so and la_loi_cookie(str(loi)):
                ghi("    " + giai_thich_loi_cookie(str(loi)) + ".")
            continue
        if payload or cuoi:
            if not nac.tran and payload:
                ghi("    ✓ qua được bằng nấc «{0}».".format(nac.ten))
            bao_nac(nac)
            return payload
        # Rỗng mà không ném lỗi: `ignoreerrors=True` nuốt mất lỗi rồi. Đây đúng
        # là bộ mặt của cái chặn đêm 22/09 — "qua cửa" nhưng không có gì trong
        # tay. Leo tiếp nấc sau chứ đừng trả về tay không.
        ghi("    nấc «{0}» không lấy được gì.".format(nac.ten))
    bao_nac(danh_sach[-1])
    return {}
