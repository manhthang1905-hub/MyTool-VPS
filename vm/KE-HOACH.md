...
**Vì sao:** đêm 22/09/2026 cả ba kênh chết ở khâu ĐẦU của sản xuất
(`kich-ban`, `core/auto_khau.py` ~2100). Khâu ấy cần LỜI THOẠI video đối thủ,
lấy qua `core.script_video.lay_script` — và `yt-dlp` lẫn
`youtube-transcript-api` đều bị YouTube chặn theo ĐỊA CHỈ MẠNG của VPS này
(`IpBlocked`, nguyên văn *"You are doing requests from an IP belonging to a
cloud provider"*), chặn nặng dần THEO SỐ LƯỢT HỎI. Cookie, đổi
`player_client`, nâng yt-dlp, proxy (`core/mang_youtube.py`): không cứu được
đường tải tiếng.

Nhưng từ ĐÚNG địa chỉ ấy, trình duyệt của kênh vào YouTube bình thường — đêm
21/09/2026 chạy trọn 4 phiên không lỗi. Nên chữ đi đường đó.

**Hai dòng chảy tách nhau về thời gian** (không phải một luồng chia hai):

- **~07:30 — phiên trình duyệt ĐỔ VÀO kho.** `chay_mot_phien` thêm một bước
  PHỤ (`lay_loi_thoai`) SAU quét trang chủ, TRƯỚC đăng: hỏi trạm
  `GET /loi-thoai/can-lay?kenh=…&k=8`, mở tab video đầu kèm dấu
  `shopapi_lt=1`, rồi extension (`core/ytb_extension/loi-thoai.js` + hàng đợi
  `ltNhanBaoCao` trong `background.js`) tự đi lần lượt: mở bảng "Bản chép lời"
  → gom chữ → `POST /loi-thoai` → NGHỈ 4–8 GIÂY NGẪU NHIÊN → video kế. Agent
  chờ tối đa `K × 12` giây rồi ĐI TIẾP bất kể xong chưa — bước phụ không được
  đẩy bước đăng (có hạn giờ thật) trễ; phần đã hút nằm sẵn ở trạm vì extension
  gửi TỪNG video một, không gom tới cuối.
- **02:00 — sản xuất chỉ ĐỌC kho.** `tu_chay.py --tat-ca` vẫn headless, vẫn
  KHÔNG mở trình duyệt nào; `lay_script` đọc kho trước mọi đường mạng và trả
  nguồn `"cache-trinh-duyet"` — không tốn một lượt hỏi nào.

**Nhịp là luật sắt:** tối đa 8 video/phiên, mỗi video 4–8 giây ngẫu nhiên.
Đây là trình duyệt ĐANG ĐĂNG NHẬP tài khoản kênh — thứ duy nhất trong cả dây
chuyền còn vào được YouTube từ địa chỉ này. Cào dồn là cách nhanh nhất để mất
luôn nó, và mất nó là mất cả Studio lẫn đăng lẫn bình luận.

**Kho:** `core/loi_thoai.py` —
`CHANNEL/_NHOM/<nhóm>/loi-thoai/<video_id>.json` (kho CHUNG: 4 kênh cùng nhóm
đánh cùng pool đối thủ, một lượt lấy phục vụ cả 4), hoặc
`CHANNEL/<MÃ>/nghien-cuu/loi-thoai/` cho kênh không thuộc nhóm. Video KHÔNG có
bảng phụ đề được cất thành bản ghi rỗng có cờ `khong_co: true` — để phiên sau
thôi hỏi lại, nhưng `lay_script` vẫn đi đường mạng cũ (đường `tu-nghe` có ngày
vẫn ra chữ).

**Núm bật/tắt:** `lay_loi_thoai` (mặc định **BẬT**) và `cho_loi_thoai_giay`
(0 = tự tính) — khai ở `core/vm_cai_dat.MAC_DINH` và `agent.KHOA_TU_TOOL`, đẩy
xuống qua `GET /viec` như `tu_dang`/`tu_tra_loi_cmt`, có test canh hai đầu.

**Đọc kết quả ở đâu:** một dòng trong sổ ngày dùng chung
`workspace/tu-chay/<ngày>.md` — *"Kho lời thoại (trình duyệt kênh hút về): N
video có lời thoại · +M lấy được hôm nay · K video không có bảng phụ đề"*.
`+0 hôm nay` là tín hiệu: phiên không chạy, `lay_loi_thoai` đang tắt, hoặc
YouTube vừa đổi DOM bảng phụ đề. Nhật ký agent (`agent.log`) có dòng *"phiên
kênh X: lời thoại — … lấy được · … không có bảng phụ đề · … chưa về"*, và
nhật ký extension (popup tiện ích) có dòng của từng video.

### Phiên thật đầu tiên: 0/8 — và vì sao (22/09/2026, 09:52–10:03)

Kênh TL1-T7 ra *"0 lấy được · 0 không có bảng phụ đề · 8 chưa về"*. Nguyên nhân
nằm đúng chỗ đã khai là "phải đoán": **dấu `&shopapi_lt=1` không sống sót.**

Bằng chứng đo trực tiếp:

- Tệp phiên Chromium
  `TL1-T7/Data/profile/Default/Sessions/Session_13434519629720236` chứa CẢ HAI
  địa chỉ cho cùng một video: `…watch?v=i2EAnLd8Duo&shopapi_lt=1` và
  `…watch?v=i2EAnLd8Duo`. Tab mở ĐÚNG địa chỉ kèm dấu, rồi YouTube tự
  `replaceState` gỡ dấu để chuẩn hoá.
- `loi-thoai.js` chạy ở `document_idle` — tới lúc ấy dấu đã mất, cửa
  `if (!…has('shopapi_lt')) return;` đóng sập.
- LevelDB `Local Extension Settings/ghbmnnjooekpmoecnnnilnnbdlolhkhi` không có
  một dòng log nào chứa "lời thoại" — khớp với việc content script thoát trước
  khi background nhận được gì. **Sự im lặng ấy là nửa sau của lỗi.**
- Launcher thứ hai (`TL1-T7.exe --load-extension=… <url>`, pid 5672, con của
  agent) không đẻ `chrome.exe` nào: nó chuyển địa chỉ cho instance đang chạy
  (2604) rồi treo, tab mở trong 2604. **Đường mở tab của agent ĐÚNG**, không sửa.

**Đã sửa (extension 2.7.1, không đụng `vm/agent.py`):** sự thật "tab nào là tab
lời thoại" chuyển khỏi URL — thứ YouTube được phép viết lại — sang ba nguồn:

1. `background.js` bắt `chrome.tabs.onUpdated` (quyền `tabs` đã có, KHÔNG thêm
   `webNavigation`); `changeInfo.url` tới với địa chỉ GỐC. `ltTabId` cất cả
   xuống `chrome.storage.local` (`lt_tab`) vì RAM của service worker chết mỗi
   lần nó ngủ. `ltKetThuc` nhả tab.
2. `lt-dau.js` (tệp mới, `run_at: document_start`) cắm cờ
   `sessionStorage['shopapi_lt']` sớm hơn `replaceState`; `loi-thoai.js` đọc
   rồi xoá ngay — cờ sống đúng một lần nạp trang.
3. địa chỉ còn dấu — nết cũ, giữ làm nguồn thứ ba.

`loi-thoai.js` giờ LUÔN nói ra quyết định của nó qua `{type:'lt_log'}`, kể cả
khi nó bỏ qua — nhật ký tiện ích có dòng
`lời thoại[trang]: <mã>: chạy (background=true · session=true · url=false)`
hoặc `lời thoại[trang]: <mã>: BỎ QUA — không phải tab lời thoại (…=0)`.

**Còn để ngỏ:** chưa chạy lại phiên thật sau bản vá — toàn bộ chuỗi được kiểm
bằng `tests/test_loi_thoai_trinh_duyet.py` (45 bài, không mạng; 5 bài mới đọc
MÃ NGUỒN extension để chốt đúng ba nguồn trên), nhưng phần JS đọc
DOM YouTube thì pytest không chạm tới được. Selector đang bám (và vì sao tin
nó): `ytd-video-description-transcript-section-renderer`,
`#description-inline-expander #expand`,
`ytd-engagement-panel-section-list-renderer[target-id="engagement-panel-searchable-transcript"]`,
`ytd-transcript-segment-renderer` + `.segment-text` — toàn tên web-component
nội bộ của YouTube, một ngôn ngữ, KHÔNG phải chữ trên nút (giao diện các kênh
này là tiếng Nhật: nút ghi "文字起こしを表示"). Yếu nhất là `target-id`, nên có
đường lui tìm thẳng `ytd-transcript-renderer`.
