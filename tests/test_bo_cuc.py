...
def test_du_tam_trang_deu_dung_len_duoc(cua_so):
    ...
    Đo theo danh sách nav THẬT của máy đang chạy, không theo `TRANG` trần:
    máy có `vps.json` không dựng mười ba trang của bản máy nhà, nó dựng ba
    trang riêng (`core.che_do_vps.loc_trang`). So với `TRANG` ở đó là bắt lỗi
    một thứ không hỏng — và che mất lỗi thật, vì ba trang VPS không nằm trong
    `TRANG` nên chúng chưa bao giờ được bài này soi.
    """
    cs, _app = cua_so
    from core import che_do_vps
    from ui_qt.app import TRANG

    khai = [k for k, _bt, _nh in che_do_vps.loc_trang(GOC, TRANG)]
    thieu = [k for k in khai if k not in cs._trang]
    assert not thieu, "khai trong danh sách trang nhưng chưa dựng: {0}".format(thieu)
