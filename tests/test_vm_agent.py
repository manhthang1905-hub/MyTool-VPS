...
class TestMatVaChrome:
    ...
    def test_lenh_chrome_kem_co_nap_extension(self, tmp_path, monkeypatch):
        ...
        # ═══ HÌNH DÒNG LỆNH CUỐI CÙNG (22/09/2026) ═══
        #
        # Đo trên máy thật: Chromium 143/151 của các bộ trình duyệt kênh NHẬN
        # `--load-extension` rồi BỎ (Google vô hiệu từ bản 137), và cửa sau
        # `--disable-features=DisableLoadExtensionCommandLineSwitch` đã thử hai lần
        # KHÔNG tác dụng — nên cờ ấy bị bỏ khỏi dòng lệnh, đừng thêm lại.
        #
        # Thay nó là ba cờ DevTools: agent tự nối vào cửa sổ vừa mở và gọi
        # `Extensions.loadUnpacked` (xem `tests/test_nap_extension_devtools.py`).
        # `--load-extension` còn đó làm MỒI vô hại cho bản Chrome nào còn nhận.
        assert lenh[1] == "--remote-debugging-port=9300"
        assert lenh[2] == "--remote-allow-origins=*"
        assert lenh[3] == "--enable-unsafe-extension-debugging"
        assert lenh[4].startswith("--load-extension=") and lenh[-1] == "https://y"
    ...
    def test_giu_chrome_chet_thi_mo_lai_song_thi_thoi(self, tmp_path,
        ...
        chrome.write_bytes(b"exe")
        # Mắt cào trỏ vào thư mục RỖNG + `ghi` gom vào list: `giu_chrome` nay đi qua
        # `mo_chrome_kenh`, và bài này chỉ kiểm việc MỞ. Không bẻ hai chỗ ấy thì bài
        # kiểm (a) ghi thẳng vào `vm/agent.log` thật và (b) ngồi chờ 40 giây cổng
        # DevTools của một trình duyệt không tồn tại.
        monkeypatch.setattr(agent, "THU_MUC_TIEN_ICH", str(tmp_path / "khong-co"))
        monkeypatch.setattr(agent, "ghi", lambda _d: None)
        da_mo = []
