...
from core import cai_dat, che_do_vps
...
def _tat_tren_vps(base_dir: str) -> bool:
    """Máy này có bị CẤM cập nhật từ GitHub không — đúng khi ở chế độ VPS.

    ═══ VÌ SAO TẮT HẲN Ở ĐÂY, KHÔNG PHẢI MỘT Ô CHỌN ═══

    Chủ dự án, 21/09/2026: *"tắt tính năng update của tool từ github để tránh
    bị ghi đè phiên bản cũ"*.

    Máy VPS chạy kênh tự động mang những bản vá **viết thẳng tại chỗ và chưa
    bao giờ có trong kho GitHub** — `core/secrets.py` (tệp chưa từng được
    commit), bản vá `sys.stdout is None` cho `core/chi_so_ytb/gom.py` và
    `giai_ma.py` khi chạy bằng `pythonw`, `_ep_hien_that_su()` trong
    `shopapi_studio_qt.py`, `core/che_do_vps.loc_trang()`. Xem
    `NHAT-KY-PHAT-TRIEN.md`. Kéo bản kho về là ghi đè ngược lên chúng, và hai
    bản vá đầu vốn là thứ khiến tool mở lên được — mất chúng là tool câm.

    Dùng đúng dấu hiệu `vps.json` sẵn có (`che_do_vps.la_vps`) thay vì thêm
    một ô chọn mới: bản gửi khách không có tệp đó nên **không đổi gì cả**, và
    ở đây không có ai ngồi canh để bấm nhầm một ô chọn.
    """
    try:
        return che_do_vps.la_vps(base_dir)
    except Exception:  # noqa: BLE001 — không đọc được thì cứ để đường cũ chạy
        return False
...
class NutCapNhat:
    ...
    def _tat_vi_vps(self) -> None:
        ...
        self.nut.setToolTip(
            "Máy chạy kênh tự động: không kéo bản từ GitHub về, tránh ghi đè "
            "các bản vá chỉ có trên máy này.")
    ...
    def _bam(self) -> None:
        if _tat_tren_vps(self._app.base_dir):
            self._tat_vi_vps()
            self._app.show_message(
                "Cập nhật từ GitHub đang tắt trên máy này",
                "Máy này chạy kênh tự động và đang mang những bản vá viết "
                "thẳng tại chỗ, chưa có trên GitHub. Kéo bản trên kho về sẽ "
                "ghi đè ngược lại chúng và tool có thể không mở lên được "
                "nữa.\n\nMuốn cập nhật thật thì xoá tệp vps.json cạnh tool — "
                "nhưng hãy chép thư mục tool ra một chỗ khác trước đã.")
            return
