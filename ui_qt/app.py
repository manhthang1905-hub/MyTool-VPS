...
class CuaSoChinh(QWidget):
    ...
    def widget_duoi_ten(self):
        ...
        # ═══ MÁY VPS KHÔNG CÓ "DỰ ÁN" ═══
        #
        # Ô này là sợi dây nối bảy tab LÀM VIDEO TAY thành một video. Trên máy
        # VPS không còn tab nào trong số ấy: `tu_chay.py` tự đặt thư mục lượt
        # theo mã kênh + mã lượt, và không ai ngồi đây gõ "tên video của bạn".
        # Chủ dự án, 21/09/2026: ô "DỰ ÁN / video-dau-tien" ở cột trái là vô
        # nghĩa với máy auto. Nên ở chế độ VPS cột trái không có nó.
        if getattr(self, "_la_vps", False):
            return None
    ...
    def __init__(self, base_dir: str):
        ...
        # Chế độ VPS: bớt hẳn các tab làm nội dung thủ công (không ai ngồi làm
        # tay ở đây, xem `core.che_do_vps.KHOA_TRANG_VPS`) — máy thường (không
        # có `vps.json`) thì `loc_trang` trả nguyên danh sách, không đổi gì.
        try:
            self._la_vps = bool(che_do_vps.la_vps(base_dir))
        except Exception:  # noqa: BLE001 — thiếu mô-đun hay tệp hỏng: máy nhà
            self._la_vps = False
        self._nav = che_do_vps.loc_trang(base_dir, self.TRANG_SAN_PHAM) + tuple(self.nav_them())
        ...
        # Trên VPS nút này KHÔNG ở góc dưới trái. Chủ dự án, 21/09/2026: nút to
        # nhất của cửa sổ đang là "Cập nhật: đã tắt" — chỗ đẹp nhất dành cho một
        # thứ đang tắt. Máy này chạy kênh thật nhiều năm; cập nhật là việc vài
        # tháng một lần, nên `ui_qt/trang_may_vi.py` nhận nút ấy về chỗ kín đáo.
        if not self._la_vps:
    ...
    def _dung_cac_trang(self) -> None:
        def _tao(ten_mo_dun: str, ten_lop: str, cua_so):
            """Nạp muộn một trang — chỉ khi nó thật sự nằm trên thanh bên."""
            import importlib  # noqa: PLC0415

            mo_dun = importlib.import_module("." + ten_mo_dun, __package__)
            return getattr(mo_dun, ten_lop)(cua_so)
        ...
        xuong = {
            ...
            #
            # Chúng chỉ xuất hiện khi `core.che_do_vps.loc_trang` trả về chúng,
            # tức khi có `vps.json`. Máy nhà không thấy ba dòng này, nên nhập
            # module cũng để TRONG lambda: máy nhà không tốn công nạp.
            "dieu-khien": lambda: _tao("trang_dieu_khien", "TrangDieuKhien", self),
            "noi-dung": lambda: _tao("trang_noi_dung", "TrangNoiDung", self),
            "may-vi": lambda: _tao("trang_may_vi", "TrangMayVi", self),
        }
