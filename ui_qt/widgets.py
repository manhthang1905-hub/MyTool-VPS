...
__all__ = [
    ...
    "HangXuongDong", "HopXuongDong", "ThanhTienDo", "gio_hhmm", "lan_chay_gon",
]


def lan_chay_gon(chu) -> str:
    """Mốc "lần chạy cuối" của `schtasks` → chữ đọc được, hay `""`.

    Việc chưa chạy lần nào thì Windows không để trống: nó in mốc giả
    **`"11/30/1999 12:00:00 AM"`**. In thẳng ra màn hình thì người dùng đọc
    được "lần cuối 30/11/1999" và tưởng máy hỏng từ thế kỷ trước.
    """
    s = str(chu or "").strip()
    if not s or s.upper() in ("N/A", "NEVER"):
        return ""
    return "" if s.startswith("11/30/1999") or s.startswith("30/11/1999") else s


def gio_hhmm(chu) -> str:
    """Giờ `schtasks` in ra → `"HH:MM"`. Không đọc được thì `""`.

    ═══ VÌ SAO PHẢI CÓ HÀM NÀY ═══

    `core.lich_tu_chay.trang_thai()` trả nguyên chuỗi Windows in ra ở cột
    "Start Time", và Windows in theo ĐỊNH DẠNG VÙNG của máy. Trên VPS đang
    chạy (đo 21/09/2026) nó là **`"2:00:00 AM"`** — 12 tiếng, có AM/PM.

    Mọi nơi trong giao diện từng cắt `chuỗi[:5]`, tức lấy `"2:00:"`. Hậu quả
    đo được trên máy thật, hai chỗ cùng lúc:

    * dải trên cùng hiện `"2:00: mỗi ngày"` — một dấu hai chấm thừa;
    * `core.trung_tam.trang_thai_bay_gio` nhận `gio_lich="2:00:"`, `_gio()`
      không dựng được giờ từ chuỗi ấy nên trả `None`, và MỌI kênh bị gắn
      **"⚠ Chưa bật lịch"** trong khi lịch `ShopAPI-TuChay` đang bật thật.

    Tệ hơn cả dấu hai chấm: `"9:04:00 PM"` cắt thành `"9:04"` là **9 giờ
    sáng**, lệch đúng nửa ngày so với 21:04 thật.

    Hàm này nhận cả ba dạng gặp được: `"HH:MM"`, `"HH:MM:SS"` và
    `"H:MM:SS AM/PM"` (kể cả `"SA"`/`"CH"` của Windows tiếng Việt).
    """
    s = str(chu or "").strip().upper().replace(".", "")
    if not s:
        return ""
    chieu = ""
    for dau, la_chieu in (("AM", False), ("PM", True), ("SA", False), ("CH", True)):
        if s.endswith(dau):
            chieu = "PM" if la_chieu else "AM"
            s = s[: -len(dau)].strip()
            break
    phan = s.split(":")
    try:
...
class HopXuongDong(QWidget):
    """Vỏ cho `HangXuongDong` khi cần **bật/tắt cả hàng** như một khối.

    `HangXuongDong` báo chiều cao theo bề rộng (`heightForWidth`). Lồng layout
    trong layout thì Qt tự hỏi câu đó và mọi thứ đúng. Nhưng bọc nó vào một
    `QWidget` thì QWidget **không tự chuyển tiếp** câu hỏi: nó báo chiều cao
    của đúng MỘT hàng, và mọi thứ xuống hàng bị cắt cụt ngoài mép dưới.

    Đo được 21/09/2026: bốn thẻ kênh của trang Trung tâm ở cửa sổ 760px xuống
    thành hai hàng, và hai thẻ hàng dưới mất nửa người — nhìn như giao diện
    hỏng. Vỏ này chuyển tiếp `heightForWidth`, thế là đủ.
    """

    def __init__(self, khoang: int = 6, cha: Optional[QWidget] = None):
        super().__init__(cha)
        self.hang = HangXuongDong(khoang)
        self.setLayout(self.hang)
        chinh_sach = self.sizePolicy()
        chinh_sach.setHeightForWidth(True)
        self.setSizePolicy(chinh_sach)

    def hasHeightForWidth(self) -> bool:  # noqa: N802 — tên do Qt quy định
        return True

    def heightForWidth(self, rong: int) -> int:  # noqa: N802
        return self.hang.heightForWidth(rong)

    def resizeEvent(self, su_kien) -> None:  # noqa: N802
        """Hẹp lại thì CAO lên — và phải nói ra bằng chiều cao tối thiểu.

        `heightForWidth` mới chỉ là *đề nghị*: khi cả trang thiếu chỗ, Qt vẫn
        ép mọi thứ về chiều cao TỐI THIỂU, mà tối thiểu của một hàng xuống dòng
        lại là đúng một hàng — nên hàng thứ hai bị cắt. Khai thẳng chiều cao
        thật làm mức tối thiểu thì trang cao thêm và vùng cuộn lo phần còn lại.
        """
        super().resizeEvent(su_kien)
        self.setMinimumHeight(self.hang.heightForWidth(self.width()))

    def addWidget(self, w: QWidget) -> None:  # noqa: N802
        self.hang.addWidget(w)

    def removeWidget(self, w: QWidget) -> None:  # noqa: N802
        self.hang.removeWidget(w)
