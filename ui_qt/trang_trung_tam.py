...
═══ BỐN KHỐI, TRÊN XUỐNG DƯỚI ═══
    ...
    A. Dải trên cùng: ổ đĩa · ví · lịch hằng ngày · tiền tiêu · các máy (VPS)
       + MỘT dòng "Cần bạn xử lý" gom mọi thứ đang hỏng, hiện ra khi có
    B. Kênh: mỗi kênh MỘT THẺ đặt cạnh nhau (đổi sang Bảng được)
    C. Kênh đang chọn: Tiến độ · Chờ duyệt · Hiệu quả · Nhật ký · Cài đặt
    D. Nhóm kênh (gập được): video thắng của từng kênh + cảnh báo trùng

═══ VÌ SAO THẺ CHỨ KHÔNG PHẢI DÒNG BẢNG ═══

Chủ dự án, 21/09/2026: *"4 kênh đặt cạnh nhau dễ so sánh"*. Sắp có bốn kênh
(TL1..TL4-T7) và câu hỏi mỗi sáng là "kênh nào hôm nay có chuyện?" — câu ấy trả
lời bằng cách liếc bốn khối cạnh nhau, không phải đọc dọc chín cột.

Thẻ còn làm được thứ dòng bảng không làm được: **đổi cả nền và viền** theo mức
(bình thường / đáng chú ý / hỏng), nên kênh hỏng nhảy vào mắt từ xa. Bảng cũ
KHÔNG bị bỏ — nó vẫn nằm đây sau nút "Bảng", vì so chín con số giữa các kênh
thì bảng vẫn hơn, và nó là chỗ lọc/sắp xếp quen tay.

Màu không bao giờ đi một mình: mỗi thẻ luôn có CHỮ nói rõ đang sao ("Đang làm
video", "Lỗi: mạng chập"), cho người không phân biệt được màu.
...
from PyQt5.QtCore import QDate, QEvent, QSize, Qt, QTime, QTimer, pyqtSignal
from PyQt5.QtGui import QBrush, QColor, QIcon, QPixmap
from PyQt5.QtWidgets import (
    QAbstractItemView, QCheckBox, QComboBox, QDateEdit, QDialog, QFrame, QLabel,
    QScrollArea, QHBoxLayout, QHeaderView, QLineEdit, QMenu, QMessageBox,
    QPlainTextEdit, QProgressBar, QRadioButton, QSpinBox,
    QStackedWidget, QTableWidget, QTableWidgetItem, QTabWidget, QTimeEdit,
    QVBoxLayout, QWidget,
)
...
from . import theme
from .widgets import (ChonThuMuc, HangXuongDong, HopXuongDong, NhomChon,
                      gio_hhmm, lan_chay_gon, mo_thu_muc, nhan, nut_chinh,
                      nut_phu, the)

__all__ = ["TrangTrungTam", "TheKenh", "HopThemKenh", "HopCaiDatMay", "KhoiLich"]
...
_MAU_MUC = {"dang": theme.NHAN, "cho": theme.VANG, "ok": theme.XANH, "loi": theme.DO,
            "canh_bao": theme.CAM, "nghi": theme.CHU_MO, "tat": theme.XAM}
_NEN_LOI = theme.DO_NEN

# ── Ba mức báo động ──────────────────────────────────────────────────────────
#
# `core.trung_tam` trả bảy mức cho cột "Bây giờ" (dang/cho/ok/loi/canh_bao/
# nghi/tat) — bảy mức là để KỂ, không phải để BÁO ĐỘNG. Người ngồi trước máy chỉ
# cần ba câu trả lời: *ổn*, *để mắt*, *hỏng rồi*. Gộp xuống ba ở đây.
THUONG, LUU_Y, HONG = "thuong", "luu_y", "hong"

#: Mức "Bây giờ" nào là báo động. Mức nào không có tên ở đây là bình thường.
#: "canh_bao" gồm cả lượt KẸT (dở ở một khâu, xong mà chưa bàn giao, quá giờ
#: đăng) — đúng những thứ chủ dự án muốn thấy màu vàng.
_MUC_BAO = {"loi": HONG, "canh_bao": LUU_Y}

#: Mỗi mức ba màu: chữ · nền · viền. Lấy từ `theme`, không tự chế.
_MAU_BAO = {
...
#: Chữ đi kèm màu — người không phân biệt được màu vẫn đọc ra mức.
_CHU_BAO = {THUONG: "", LUU_Y: "⚠", HONG: "✕"}

#: Ổ đĩa. Máy đang chạy chỉ còn ~13 GB trống, mỗi video ngốn 0,4–0,9 GB trước
#: khi dọn — hết chỗ là mọi kênh đứng hình cùng lúc, nên đây là rủi ro to nhất.
GB_DIA_HONG = 10
GB_DIA_LUU_Y = 25

#: Ví. Một video cỡ 16 phút khoảng 80–100 nghìn, nên dưới 100 nghìn là không đủ
#: cho một video nữa; dưới 300 nghìn là không đủ cho bốn kênh chạy một ngày.
VND_VI_HONG = 100_000
VND_VI_LUU_Y = 300_000

#: Tên hiện ra của các máy trong VPS (khoá do `core.giam_sat_vm` đặt).
#:
#: ═══ HAI KHOÁ THIẾU ĐÃ LÀM LỘ TÊN KỸ THUẬT RA MÀN HÌNH ═══
#:
#: `core.giam_sat_vm.CAC_CON` đặt tên ba con là `agent`, **`tu_dang`** và
#: **`tu_tra_loi_cmt`** — không phải `may_dang`/`may_cmt`. Bảng này thiếu đúng
#: hai khoá ấy, nên `_TEN_MAY.get(ten, ten)` rơi về chính cái khoá, và ảnh chụp
#: cửa sổ thật ngày 21/09/2026 có hai ô ghi "tu_dang" và "tu_tra_loi_cmt" nằm
#: giữa dải số liệu. `MyTool/CLAUDE.md` cấm đúng điều này: *"Không dùng từ kỹ
#: thuật trên giao diện"*. Giữ luôn `may_dang`/`dang`/`may_cmt`/`cmt` cho bản
#: `vm/giao_dien.py` cũ và cho bài kiểm đang dùng chúng.
_TEN_MAY = {"agent": "Agent",
            "tu_dang": "Máy đăng", "may_dang": "Máy đăng", "dang": "Máy đăng",
...
def _muc_bao(muc_bay_gio: str) -> str:
    """Mức "Bây giờ" của `core.trung_tam` → một trong ba mức báo động."""
    return _MUC_BAO.get(str(muc_bay_gio or ""), THUONG)


def _nang(a: str, b: str) -> str:
    """Mức nặng hơn trong hai mức — gom nhiều nguồn báo về một mức chung."""
    thu_tu = {THUONG: 0, LUU_Y: 1, HONG: 2}
    return a if thu_tu.get(a, 0) >= thu_tu.get(b, 0) else b


def _cat(nhan_chu: QLabel, chu: str, rong: int) -> None:
    """Đặt chữ đã cắt theo PIXEL. Cắt theo số ký tự là sai với chữ Nhật: 20 ký
    tự Nhật rộng gấp đôi 20 ký tự Việt, và đúng chỗ ấy thẻ phình quá mép."""
    nhan_chu.setText(nhan_chu.fontMetrics().elidedText(str(chu or ""), Qt.ElideRight, rong))


class _OChiSo(QFrame):
    """Một ô số liệu trên dải trên cùng: **tên ở trên, số ở dưới**.

    Trước đây dải này là một hàng chữ nhỏ bằng nhau — ví, tiền, ổ đĩa, lịch
    trông y hệt nhau nên không cái nào nổi. Tách hai dòng thì con số to lên gấp
    rưỡi và đọc được từ xa, còn cái tên nhỏ lại vì người ta chỉ đọc nó một lần.

    Ô đổi CẢ chữ lẫn màu theo mức, không chỉ màu.
    """

    clicked = pyqtSignal()

    def __init__(self, tieu_de: str, cha: Optional[QWidget] = None):
        super().__init__(cha)
        self._tieu = tieu_de
        self.setObjectName("oChiSo")
        self.setCursor(Qt.PointingHandCursor)
        doc = QVBoxLayout(self)
        doc.setContentsMargins(11, 5, 11, 5)
        doc.setSpacing(0)
        self._nhan_tieu = QLabel(tieu_de)
        self._nhan_tieu.setWordWrap(False)
        self._nhan_tieu.setStyleSheet(
            "color:{0};font-size:11px;".format(theme.CHU_MO))
    ...
    def dat(self, gia_tri: str, muc: str = THUONG, tip: str = "") -> None:
        chu, nen, vien = _MAU_BAO[muc]
        dau = _CHU_BAO[muc]
        self._gia = str(gia_tri)
        self._nhan_gia.setText((dau + " " if dau else "") + self._gia)
        self._nhan_gia.setStyleSheet(
            "color:{0};font-size:14px;font-weight:600;".format(chu))
        self.setStyleSheet(
            "QFrame#oChiSo{{background:{0};border:1px solid {1};border-radius:10px;}}"
            .format(nen, vien))
        if tip:
            self.setToolTip(tip)

    def text(self) -> str:
        """Cả hai dòng — để bài kiểm đọc được nội dung ô như một nhãn thường."""
        return "{0}\n{1}".format(self._tieu, self._gia)
...
class TheKenh(QFrame):
    """MỘT kênh = MỘT thẻ. Bốn thẻ đứng cạnh nhau là bốn kênh so được bằng mắt.

    Thẻ tự xuống dòng khi cửa sổ hẹp (nằm trong `HangXuongDong`), nên một kênh
    hay bốn kênh đều gọn: một kênh là một thẻ đứng riêng, bốn kênh là một hàng
    bốn thẻ, cửa sổ hẹp thì thành hai hàng hai thẻ.
    """

    #: Bề rộng một thẻ. Đo ngược từ cửa sổ 1280 của máy thật: trừ thanh bên
    #: 240, lề trang 2×20, lề thẻ bọc 2×12 và thanh cuộn thì còn ~966px cho bốn
    #: thẻ và ba khoảng cách 8px — nên một thẻ tối đa 235px. Rộng hơn là bốn
    #: kênh tụt xuống hai hàng ngay trên chính cái máy này.
    RONG = 232
    #: Cột tên ở đầu mỗi dòng số liệu ("Kiếm tiền" là nhãn dài nhất).
    _RONG_TEN = 58
    #: Bề rộng còn lại cho phần giá trị.
    _RONG_GIA = RONG - 16 - _RONG_TEN - 6

    chon_kenh = pyqtSignal(str)
    mo_tien_do = pyqtSignal(str)
    doi_tu_chay = pyqtSignal(str, bool)

    #: Các dòng số liệu: (khoá, nhãn NGẮN, lời giải thích cho tooltip).
    DONG = (
        ("video", "Hôm nay", "Video kênh này đang làm hoặc vừa làm xong hôm nay."),
        ("dang", "Đăng", "Lúc video lên sóng."),
        ("bay_ngay", "7 ngày", "Tổng lượt xem · tỷ lệ bấm của video đăng trong 7 ngày."),
        ("ypp", "Kiếm tiền", "Đường tới mốc bật kiếm tiền: 4.000 giờ xem · 1.000 người đăng ký."),
        ("tien", "Tiền", "Hôm nay đã tiêu (ước tính) / trần mỗi ngày."),
    )
    ...
    def __init__(self, ma: str, cha: Optional[QWidget] = None):
        ...
        self._chon = False
        self._muc = THUONG
        self.setObjectName("theKenh")
        self.setFixedWidth(self.RONG)
        self.setCursor(Qt.PointingHandCursor)
        doc = QVBoxLayout(self)
        doc.setContentsMargins(8, 7, 8, 7)
        doc.setSpacing(3)

        self._nhan_ten = QLabel(ma)
        self._nhan_ten.setWordWrap(False)
        self._nhan_ten.setStyleSheet("font-size:14px;font-weight:700;")
        doc.addWidget(self._nhan_ten)

        self._nhan_tt = QLabel("")
        self._nhan_tt.setWordWrap(False)
        doc.addWidget(self._nhan_tt)

        self._gia: Dict[str, QLabel] = {}
        for khoa, ten, tip in self.DONG:
            hang = QHBoxLayout()
            hang.setContentsMargins(0, 0, 0, 0)
            hang.setSpacing(6)
            nh = QLabel(ten)
            nh.setFixedWidth(self._RONG_TEN)
            nh.setWordWrap(False)
            nh.setStyleSheet("color:{0};font-size:11px;".format(theme.CHU_MO))
            nh.setToolTip(tip)
            gt = QLabel("—")
            gt.setWordWrap(False)
            gt.setStyleSheet("font-size:12px;")
            gt.setToolTip(tip)
            hang.addWidget(nh)
            hang.addWidget(gt, 1)
            doc.addLayout(hang)
        ...
        self._o_tu_chay = QCheckBox("Tự chạy")
        self._o_tu_chay.setStyleSheet("font-size:12px;")
        self._o_tu_chay.setToolTip("Cho kênh này tự làm video mỗi ngày.")
        self._o_tu_chay.toggled.connect(
            lambda bat: self.doi_tu_chay.emit(self._ma, bool(bat)))
        doc.addWidget(self._o_tu_chay)
        self._ve_vien()
    ...
    def nap(self, k: Dict[str, Any]) -> None:
        ...
        ten = k.get("ten") or k.get("ma") or ""
        day_du = k["ma"] if ten == k["ma"] else "{0} · {1}".format(k["ma"], ten)
        _cat(self._nhan_ten, day_du, self.RONG - 16)
        # Tệp khán giả không chiếm một dòng trên thẻ: nó gần như không đổi, nên
        ...
        mau_chu = _MAU_MUC.get(str(bay.get("muc") or ""), theme.CHU) \
            if self._muc == THUONG else chu
        # Trừ cả lề thẻ (16), đệm trong ô (14) và viền (4) — cắt rộng hơn thì
        # chữ chạm mép ô màu.
        _cat(self._nhan_tt, (dau + " " if dau else "") + str(bay.get("chu") or "—"),
             self.RONG - 34)
        self._nhan_tt.setToolTip(bay.get("chi_tiet") or bay.get("chu") or "")
        self._nhan_tt.setStyleSheet(
            "QLabel{{background:{0};border:1px solid {1};border-radius:7px;"
            "padding:2px 7px;color:{2};font-size:12px;font-weight:600;}}".format(
                nen if self._muc != THUONG else theme.NEN,
                vien if self._muc != THUONG else theme.VIEN, mau_chu))

        td = (k.get("video") or {}).get("tieu_de") or ""
        self._dat("video", td or "—", td)
        self._dat("dang", k.get("dang_luc") or "—", "")
        ...
        self._dat("bay_ngay", chu7, tip7)

        y = k.get("ypp") or {}
        co_y = y.get("gio_xem") is not None or y.get("dang_ky") is not None
        self._dat("ypp",
                  "{0} giờ · {1} ĐK".format(_gon(y.get("gio_xem")), _gon(y.get("dang_ky")))
                  if co_y else "—",
                  "Đường bật kiếm tiền: {0}/4.000 giờ xem · {1}/1.000 người đăng ký.".format(
                      _gon(y.get("gio_xem")), _gon(y.get("dang_ky"))))

        tien = k.get("tien") or {}
        self._dat("tien", "{0} / {1}".format(
            _vnd_gon(tien.get("hom_nay")),
            _vnd_gon(tien.get("tran")) if tien.get("tran") else "chưa đặt"),
            "Hôm nay đã tiêu {0} (ước tính) / trần mỗi ngày {1}.".format(
                _vnd(tien.get("hom_nay")),
                _vnd(tien.get("tran")) if tien.get("tran") else "chưa đặt"))

        # Đặt ô tick mà KHÔNG bắn tín hiệu: đây là vẽ lại, không phải người bấm.
        self._o_tu_chay.blockSignals(True)
        self._o_tu_chay.setChecked(bool(k.get("tu_chay")))
        self._o_tu_chay.blockSignals(False)
        self._ve_vien()

    def _dat(self, khoa: str, chu: str, tip: str) -> None:
        nh = self._gia[khoa]
        _cat(nh, chu, self._RONG_GIA)
        nh.setToolTip(tip or str(chu))
    ...
    def _ve_vien(self) -> None:
        _chu, nen, vien = _MAU_BAO[self._muc]
        # Viền LUÔN 2px: đổi độ dày khi chọn làm cả hàng thẻ nhích một pixel.
        self.setStyleSheet(
            "QFrame#theKenh{{background:{0};border:2px solid {1};border-radius:12px;}}"
            .format(theme.THE if self._muc == THUONG else nen,
                    theme.NHAN if self._chon else vien))
    ...
    def mouseDoubleClickEvent(self, su_kien) -> None:  # noqa: N802 — tên do Qt
        self.mo_tien_do.emit(self._ma)
        super().mouseDoubleClickEvent(su_kien)
...
class KhoiLich(QWidget):
    ...
    def ve(self, tt_lich) -> None:
        ...
        if tt_lich.get("da_dang_ky"):
            # `gio_hhmm`, KHÔNG phải `[:5]` — Windows in giờ theo định dạng
            # vùng của máy ("2:00:00 AM"), cắt năm ký tự ra "2:00:".
            gio = gio_hhmm(tt_lich.get("gio"))
            chu = "Lịch hằng ngày: BẬT, {0} mỗi ngày".format(gio or "?")
            cuoi = lan_chay_gon(tt_lich.get("lan_chay_cuoi"))
            if cuoi:
                chu += " — lần cuối {0} ({1})".format(
                    cuoi, tt_lich.get("ket_qua_cuoi") or "")
...
class TrangTrungTam(QWidget):
    def __init__(self, app):
        ...
        doc.addWidget(self._dai_canh_bao())
    ...
    def _dai_tren(self):
        """Dải ô số liệu của CÁI MÁY. Thứ tự = thứ tự gây thiệt hại.

        Ổ đĩa đứng đầu: máy này chỉ còn hơn chục GB, và hết chỗ thì MỌI kênh
        đứng hình cùng một lúc — nặng hơn mọi thứ khác trên dải.
        """
        hang = HangXuongDong(8)

        self._nhan_dia = _OChiSo("Ổ đĩa còn")
        self._nhan_dia.setToolTip("Chỗ trống còn lại trên ổ đặt tool. Mỗi video "
                                  "chiếm khoảng 0,4–0,9 GB trước khi dọn. Bấm để mở thư mục.")
        self._nhan_dia.clicked.connect(lambda: mo_thu_muc(self._app.base_dir))
        hang.addWidget(self._nhan_dia)

        self._nhan_vi = _OChiSo("Ví")
        self._nhan_vi.setToolTip("Số dư ví. Hết tiền là kênh ngừng sản xuất. "
                                 "Bấm để mở trang Tài khoản.")
        self._nhan_vi.clicked.connect(lambda: self._mo_tai_khoan(0))
        hang.addWidget(self._nhan_vi)

        self._nut_lich = _OChiSo("Lịch hằng ngày")
        ...
        self._nhan_tien = _OChiSo("Tiền đã tiêu")
    ...
    def _dai_canh_bao(self) -> QWidget:
        """MỘT dòng gom mọi thứ đang hỏng, ngay dưới dải số liệu.

        Bốn kênh, bốn máy con, ví, ổ đĩa và lịch là chín chỗ có thể hỏng. Bắt
        người ta tự quét chín chỗ mỗi sáng là cách chắc chắn để họ bỏ sót đúng
        chỗ đang cháy. Dòng này nói thẳng có mấy việc và là việc gì; không có
        việc nào thì nó biến mất chứ không đứng đó khoe chữ "mọi thứ ổn".
        """
        self._nhan_bao = nhan("")
        self._nhan_bao.setWordWrap(True)
        self._nhan_bao.setMinimumWidth(1)
        self._nhan_bao.setVisible(False)
        return self._nhan_bao

    def _ve_canh_bao(self) -> None:
        """Gom mức nặng nhất của từng nguồn thành một câu."""
        viec: List[str] = []
        muc = THUONG
        for m, chu in self._viec_can_lam():
            muc = _nang(muc, m)
            viec.append(chu)
        self._nhan_bao.setVisible(bool(viec))
        if not viec:
            return
        chu_mau, nen, vien = _MAU_BAO[muc]
        self._nhan_bao.setText("{0} {1} việc cần bạn xem: {2}".format(
            _CHU_BAO[muc], len(viec), " · ".join(viec)))
        self._nhan_bao.setStyleSheet(
            "QLabel{{background:{0};border:1px solid {1};border-radius:10px;"
            "padding:6px 11px;color:{2};font-size:13px;font-weight:600;}}".format(
                nen, vien, chu_mau))

    def _viec_can_lam(self) -> List[tuple]:
        """Danh sách (mức, câu ngắn) — thuần, bài kiểm gọi thẳng được."""
        ra: List[tuple] = []
        anh = self._anh or {}

        con = (anh.get("o_dia") or {}).get("con_gb")
        if con is not None and con < GB_DIA_LUU_Y:
            ra.append((HONG if con < GB_DIA_HONG else LUU_Y,
                       "Ổ đĩa chỉ còn {0:.0f} GB".format(con)))

        muc_vi = self._muc_vi()
        if muc_vi != THUONG:
            ra.append((muc_vi, "Ví chỉ còn {0}".format(
                format_vnd(getattr(self._app, "last_wallet_micro", None)))))

        if self._gio_lich() == "" and any(k.get("tu_chay") for k in anh.get("kenh") or []):
            ra.append((LUU_Y, "Lịch hằng ngày đang tắt"))

        if self._co_may():
            for ten, tt_may in self._may.items():
                if not (tt_may or {}).get("song"):
                    ra.append((HONG, "{0} đã tắt".format(_TEN_MAY.get(ten, ten))))

        for k in anh.get("kenh") or []:
            m = _muc_bao((k.get("bay_gio") or {}).get("muc"))
            if m != THUONG:
                ra.append((m, "{0}: {1}".format(k.get("ma"),
                                                (k.get("bay_gio") or {}).get("chu") or "")))
        return ra

    def _muc_vi(self) -> str:
        """Ví thấp tới mức nào. Chưa đăng nhập / chưa biết số dư thì không kêu."""
    ...
    def _ve_dia(self) -> None:
        ...
        con = dia.get("con_gb")
        if con is None:
            self._nhan_dia.dat("chưa đọc được")
            return
        muc = HONG if con < GB_DIA_HONG else (LUU_Y if con < GB_DIA_LUU_Y else THUONG)
        them = {HONG: " — sắp đầy", LUU_Y: " — hơi chật", THUONG: ""}[muc]
        self._nhan_dia.dat("{0:.0f} GB{1}".format(con, them), muc)

    def _co_may(self) -> bool:
        """Máy này có các máy con để trông không. Dải đèn và dòng "Cần bạn xem"
        phải hỏi CÙNG một câu, không thì đèn tắt mà dòng kia vẫn kêu."""
    ...
    def _gio_lich(self) -> Optional[str]:
        ...
        ═══ LỖI ĐO ĐƯỢC 21/09/2026 ═══

        Chỗ này từng cắt `[:5]`. Trên máy thật `schtasks` in ra `"2:00:00 AM"`,
        cắt năm ký tự được `"2:00:"` — và `core.trung_tam._gio()` không dựng
        lại được giờ từ chuỗi ấy, nên `trang_thai_bay_gio` gắn **"⚠ Chưa bật
        lịch"** lên MỌI kênh trong khi `ShopAPI-TuChay` đang bật thật.
        Xem `ui_qt/widgets.py::gio_hhmm`.
        """
    ...
    def _ve_lich(self) -> None:
        gio = self._gio_lich()
        if gio is None:
            self._nut_lich.dat("đang xem…")
        elif gio:
            self._nut_lich.dat("{0} mỗi ngày".format(gio))
        else:
            # Lịch tắt chỉ đáng kêu khi CÓ kênh chờ nó chạy.
            co_kenh = any(k.get("tu_chay") for k in (self._anh or {}).get("kenh") or [])
            self._nut_lich.dat("ĐANG TẮT", LUU_Y if co_kenh else THUONG,
                               "Lịch đang tắt — kênh không tự chạy khi bạn vắng. Bấm để bật.")

    def _ve_vi(self) -> None:
        if getattr(self._app, "client", None) is None:
            self._nhan_vi.dat("chưa đăng nhập", LUU_Y,
                              "Chưa đăng nhập thì kênh không sản xuất được. Bấm để mở trang Tài khoản.")
            return
        micro = getattr(self._app, "last_wallet_micro", None)
        self._nhan_vi.dat(format_vnd(micro) if micro is not None else "—", self._muc_vi())
        ...
        def xong(so_du) -> None:
            ...
            self._nhan_vi.dat(format_vnd(wallet_micro(so_du)), self._muc_vi())
            self._ve_canh_bao()

        self._app.run_bg(viec, on_ok=xong, on_err=lambda _l: None)

    # ── B. Kênh: thẻ cạnh nhau (hoặc bảng) ───────────────────────────────────
    ...
    def _khoi_bang(self) -> QWidget:
        ...
        # Nhãn NGẮN (CLAUDE.md): "Thẻ" / "Bảng", giải thích nằm ở tooltip.
        self._chon_kieu = NhomChon(("Thẻ", "Bảng"), "Thẻ", on_change=self._doi_kieu_b)
        self._chon_kieu.setToolTip("Thẻ: mỗi kênh một ô, đặt cạnh nhau cho dễ so sánh.\n"
                                   "Bảng: mỗi kênh một dòng, xem được nhiều con số một lúc.")
        tren.addWidget(self._chon_kieu)
        ...
        # Thẻ kênh — hàng tự xuống dòng khi cửa sổ hẹp, nên bề rộng tối thiểu
        # của cả trang chỉ bằng MỘT thẻ.
        self._hop_the = HopXuongDong(8)
        self._hang_the = self._hop_the.hang
        self._the_kenh: Dict[str, TheKenh] = {}
        v.addWidget(self._hop_the)
        ...
        self._bang_kenh.setVisible(False)
    ...
    def _doi_kieu_b(self, kieu: str) -> None:
        """Đổi giữa Thẻ và Bảng.

        Bảng ẨN chứ không bị xoá: nó vẫn là chỗ giữ dòng đang chọn và ô tick
        "Tự chạy" của mọi kênh, nên mọi thứ đang chạy được vẫn chạy y như cũ dù
        người dùng không bao giờ bấm sang Bảng.
        """
        if not self._con_song():
            return
        bang = kieu == "Bảng"
        self._bang_kenh.setVisible(bang)
        self._hop_the.setVisible(not bang)
        if bang:
            self._chinh_cao_bang()

    def _ve_the(self, kenh: List[Dict[str, Any]]) -> None:
        """Dựng/nạp lại thẻ kênh. Thẻ được GIỮ LẠI giữa hai lần làm mới — dựng
        lại 4 thẻ mỗi 30 giây thì thẻ nhấp nháy và ô tick mất trạng thái."""
        con = {k["ma"] for k in kenh}
        for ma in [m for m in self._the_kenh if m not in con]:
            the_cu = self._the_kenh.pop(ma)
            self._hang_the.removeWidget(the_cu)
            the_cu.setParent(None)
            the_cu.deleteLater()
        for k in kenh:
            ma = k["ma"]
            t = self._the_kenh.get(ma)
            if t is None:
                t = TheKenh(ma, self._hop_the)
                t.chon_kenh.connect(self._chon_kenh)
                t.mo_tien_do.connect(self._mo_tien_do)
                t.doi_tu_chay.connect(self._dat_tu_chay)
                self._hang_the.addWidget(t)
                self._the_kenh[ma] = t
            t.nap(k)
            t.setVisible(True)
        self._hop_the.setVisible(self._chon_kieu.get() != "Bảng")

    def _chon_kenh(self, ma: str) -> None:
        """Bấm một thẻ = chọn kênh đó. Đi qua bảng để CHỈ CÓ MỘT chỗ giữ dòng
        đang chọn — hai nơi cùng nhớ là hai nơi lệch nhau."""
        if not self._con_song():
            return
        for r in range(self._bang_kenh.rowCount()):
            it = self._bang_kenh.item(r, 0)
            if it is not None and str(it.data(Qt.UserRole)) == ma:
                self._bang_kenh.selectRow(r)
                return

    def _mo_tien_do(self, ma: str) -> None:
        self._chon_kenh(ma)
        self._tabs.setCurrentIndex(0)

    def _ve_the_chon(self) -> None:
        for ma, t in self._the_kenh.items():
            t.dat_chon(ma == self._ma_chon)
    ...
    def _doi_o_tu_chay(self, item: QTableWidgetItem) -> None:
        ...
        if not ma:
            return
        self._dat_tu_chay(ma, item.checkState() == Qt.Checked)

    def _dat_tu_chay(self, ma: str, bat: bool) -> None:
        """Bật/tắt tự chạy cho một kênh. MỘT đường ghi duy nhất cho cả ô tick
        trên thẻ lẫn ô tick trong bảng — hai đường ghi là hai lần lệch nhau."""
        if self._dang_ve_bang or not self._con_song() or not ma:
            return
        try:
    ...
    def _nhan_anh(self, ket) -> None:
        ...
        self._nhan_tien.dat("~{0} hôm nay · ~{1} tháng này".format(
