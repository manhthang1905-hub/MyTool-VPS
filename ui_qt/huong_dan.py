...
HUONG_DAN: Dict[str, Dict[str, object]] = {
    ...
    # ── Ba trang của MÁY VPS (21/09/2026) ────────────────────────────────────
    #
    # Chỉ hiện khi máy có `vps.json` (`core/che_do_vps.py`). Máy nhà không thấy
    # ba trang này, nhưng ba bài dưới đây vẫn nằm đây — bài hướng dẫn tra theo
    # KHOÁ, không theo danh sách tab, nên để sẵn thì không trang nào mở ra mà
    # nút “Hướng dẫn” lại biến mất.
    "dieu-khien": {
        "tieu_de": "Điều khiển",
        "tom_tat": "Màn hình mở đầu của máy này: máy còn sống không, mỗi kênh "
                   "đang ở khâu nào của dây chuyền, và mọi công tắc để dừng "
                   "hay bật lại. Tự đọc lại tệp trên máy mỗi 30 giây — không "
                   "tốn tiền, không hỏi máy chủ.",
        "buoc": [
            "Dải trên cùng: ổ đĩa, ví, tiền đã tiêu hôm nay, và ba máy chạy "
            "nền (Agent, Máy đăng, Trả lời bình luận) còn sống không. Bấm "
            "một ô máy để bật lại hoặc xem nhật ký.",
            "Dòng “N việc cần xem” chỉ hiện khi CÓ việc — bấm để mở danh "
            "sách, việc hỏng (✕) đứng trên việc đáng lưu ý (⚠).",
            "Mỗi kênh một CỘT, đầu cột là một dòng trạng thái — viền cột đổi "
            "màu theo đúng dòng đó, đỏ là đang hỏng nặng. SÁNG THẤY CỘT ĐỎ: "
            "rê chuột lên dòng đó đọc lý do, rồi tìm khâu đang ✕ bên dưới.",
            "Tám khâu một video, từ Kịch bản tới Dựng video: ✓ xong · ◑ đang "
            "chạy · ○ chờ · ✕ hỏng — rê chuột lên một khâu đọc lý do. Sửa "
            "xong thì bấm “Chạy lại”: làm tiếp từ chỗ hỏng, không mất phần "
            "đã xong.",
            "Bốn công tắc dưới mỗi cột: Tự chạy, Tự đăng, Trả lời bình luận, "
            "Tự dọn — gạt là lưu ngay.",
            "Ô “Ngân sách” là trần tiền MỖI NGÀY, sửa ngay tại chỗ. “Chạy "
            "ngay” làm luôn, không chờ lịch.",
            "Muốn dừng hết máy: bấm “Tạm dừng tất cả” — tắt tự chạy của MỌI "
            "kênh. Bấm lần nữa (nút đổi thành “Bật lại tất cả”) là bật lại "
            "ĐÚNG những kênh nó vừa tắt.",
        ],
        "luu_y": [
            "Trần tiền 0₫ KHÔNG phải “bỏ trần” — nghĩa là KHÔNG sản xuất gì "
            "cả. Muốn không giới hạn thì đặt một số rất cao.",
            "“Tạm dừng tất cả” không giết lượt đang chạy dở; lịch hằng ngày "
            "cũng chỉ chạy khi máy ĐANG BẬT và ĐÃ ĐĂNG NHẬP đúng giờ.",
            "Tên kênh tiếng Nhật dài nên cột chỉ hiện mã kênh và tên tệp "
            "khán giả; rê chuột lên để đọc tên đầy đủ.",
        ],
    },
    "noi-dung": {
        "tieu_de": "Nội dung",
        "tom_tat": "Mọi thứ về CÁI VIDEO: hàng chờ duyệt, dây chuyền sản xuất, "
                   "nghiên cứu đối thủ, và cài đặt của từng kênh.",
        "buoc": [
            "Duyệt & đăng: video đã làm xong nhưng chưa có giờ lên sóng. Chọn "
            "một dòng, “Xem video” để xem lại, “Duyệt đăng” để đặt ngày giờ, "
            "hoặc “Bỏ” nếu không muốn đăng (tệp vẫn giữ nguyên).",
            "Sản xuất: dây chuyền tám khâu, chạy tay từng khâu khi cần chữa "
            "một lượt hỏng.",
            "Đối thủ: nghiên cứu, chọn nguồn và công thức V7 — nơi quyết định "
            "hôm nay kênh làm lại video nào.",
            "Quản lý kênh: MỌI thiết lập của một kênh, sửa ngay trên màn hình. "
            "Giọng đọc, trần tiền, giờ đăng, bốn công tắc, độ dài mục tiêu, "
            "nhãn tiêu đề, số bản nháp, bộ vẽ và màu chữ ảnh bìa.",
        ],
        "luu_y": [
            "“Tự đặt giờ đăng” nghĩa là video LÊN SÓNG THẬT mà không ai xem "
            "lại. Chỉ bật khi bạn đã tin kịch bản và hình của kênh đó.",
            "Các kênh cùng nhóm phải khác giọng đọc, khác nhân vật, khác bộ "
            "vẽ — trùng thì với khán giả chúng là MỘT kênh đăng hai lần. Ô "
            "giọng đọc tự cảnh báo khi trùng.",
            "Đổi bộ vẽ ghi đè 16 khoá hình trong style.yaml của kênh và không "
            "hoàn tác được — tool hỏi lại một câu trước khi ghi.",
        ],
    },
    "may-vi": {
        "tieu_de": "Máy & Ví",
        "tom_tat": "Cái máy và cái ví: ba máy chạy nền, nhật ký, lịch của "
                   "Windows, báo sự cố về điện thoại, số dư và nạp tiền.",
        "buoc": [
            "Ba máy chạy nền chạy TÁCH khỏi cửa sổ này — đóng tool thì chúng "
            "vẫn chạy. Bấm một ô để xem nhật ký của nó, hoặc bấm “Bật lại máy”.",
            "Lịch của Windows có hai việc: “Chạy mọi kênh lúc…” là giờ làm "
            "video hằng ngày; “Canh trạm mỗi…” là chó giữ nhà cho cổng 8765 — "
            "cổng chết thì nó bật lại, nên không ai phải ngồi canh.",
            "Báo sự cố: điền mã bot Telegram và mã hộp chat, bật ô lên rồi bấm "
            "“Gửi thử một tin”. Nhận được tin là đường đã thông; từ đó máy tự "
            "nhắn khi ví cạn, ổ đầy, trạm chết hay một kênh hỏng.",
            "Trạm & tiện ích: cổng nhận số liệu của ba máy con và phần cài "
            "tiện ích trình duyệt.",
            "Ví & Cài đặt: số dư, nạp tiền, đăng nhập, và mấy nút gạt chung "
            "của tool.",
        ],
        "luu_y": [
            "Mã bot Telegram nằm trong tệp riêng (`bao-dong.json`), KHÔNG nằm "
            "chung với khoá tài khoản của bạn.",
            "Telegram không cho bot nhắn trước: mở Telegram, nhắn cho bot của "
            "bạn một câu bất kỳ, rồi mới bấm “Gửi thử một tin”.",
            "Nút cập nhật tool nằm ở cuối trang này. Cập nhật giữa chừng có "
            "thể cắt ngang một lượt đang sản xuất hoặc một phiên đang đăng — "
            "xem trang Điều khiển trước khi bấm.",
        ],
    },
    "trung_tam": {
        "tieu_de": "Trung tâm",
